    ...
    <BottomSheet :show="isOpen" :title="label" @close="closePicker">
        <div class="py-2">
            ...
            <button
                ...
                type="button"
                @click="selectAction"
                class="w-full px-4 py-3 text-left text-sm text-primary font-medium hover:bg-gray-50"
            >
                {{ actionOption.label }}
            </button>

            <div v-if="actionOption" class="border-b border-gray-100 my-2" />

            <!-- Grouped options -->
            <template v-if="grouped">
                <div v-for="group in options" :key="group[groupLabelKey] || group.name">
                    <div class="px-4 py-2 text-xs font-semibold text-subtle uppercase tracking-wide bg-gray-50">
                    ...
                    <button
                        ...
                        class="w-full px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center justify-between"
                        :class="isSelected(item) ? 'text-primary font-medium' : 'text-body'"
                    >
                        <span>{{ getOptionLabel(item) }}</span>
                        <svg v-if="isSelected(item)" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-primary" viewBox="0 0 20 20" fill="currentColor">
            ...
            <template v-else>
                <button
                    ...
                    class="w-full px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center justify-between"
                    :class="isSelected(option) ? 'text-primary font-medium' : 'text-body'"
                >
                    <span>{{ getOptionLabel(option) }}</span>
                    <svg v-if="isSelected(option)" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-primary" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                    </svg>
                </button>
            </template>
