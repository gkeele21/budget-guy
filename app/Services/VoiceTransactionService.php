...
- Match payee names to existing payees using phonetic/fuzzy matching — speech recognition often misspells names, substitutes similar-sounding names, or drops/changes letters in proper names (e.g. "Macys" for "Macey's", "Aaron Haywood" for "Erin Heywood", "Keel" or "Kiel" for "Keele", "Costco" for "CostCo"). Be very aggressive about matching to existing payees — if a spoken name is phonetically close to an existing payee, ALWAYS use the existing payee. Pay special attention to first names (Aaron/Erin, John/Jon, Shawn/Sean) and surname spelling variants (Keel/Keele, Haywood/Heywood). If no existing payee sounds close, use the spoken name exactly as-is — a new payee will be created automatically. NEVER ask for clarification about payees. Payees are not a valid clarification field. Transfers have no payee.
- Match category names to existing categories using phonetic/fuzzy matching — speech recognition often misspells or rephrases names (e.g. "Burton Holly" for "Bert and Holly", "dining" for "Dining Out", "gas" for "Fuel"). Always prefer the closest-sounding existing category. If no match but the matched payee has a default_category_id, use that default. If still no match, set category_id to null (it will be "Unassigned") — do NOT flag for clarification. Transfers have no category.
- Match account names to existing accounts using phonetic/fuzzy matching — speech recognition often misspells names (e.g. "City Visa" for "Citi Visa", "checking" for "Checking Account"). Always match to the closest-sounding existing account. Be especially careful not to confuse account names with payee/store names (e.g. "Target" the account vs "Target" the store). If the user says "from [account]" or "the following are from [account]", that specifies the account for those transactions. If no account is mentioned and there is exactly {$accountCount} account(s), use the first one. If multiple accounts exist and none is mentioned, set account_id to null and flag for clarification.
